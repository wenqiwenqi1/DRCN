...
def transfer_classification(config,modelname,taskname,b,c):
    start=time.time()
    ## set pre-process
    prep_dict = {}
    ...
    base_network=torch.load('../save/nobottle_base_network_'+modelname+'.pkl')
    ...
    for i in range(config["num_iterations"]):
        ...
        if loss_config["name"] == "DRCN":
            ...
            while(len(savecount)!=category_num):
                savecount.append(0)

            import  copy
            for u in range(len(predict)):
                savenew.append(predict[u])
                if (len(savenew) == lentarget):
                    saveold = copy.deepcopy(savenew)
                    savenew = []
                    for o in range(len(savecount)):
                        savecount[o]=0
                    for o in range(len(saveold)):
                        savecount[saveold[o]]+=1
            ...
            for k in xrange(category_num):
                ...
                for index, target_k in enumerate(predict):
                    if (target_k == k):
                        target_index.append(index)
                conditional_loss1 = 0

                if (len(source_index) > 0 and len(target_index) > 0):
                    source_concat1 = None
                    target_concat1 = None
                    ...
                    for index in xrange(len(target_index)):
                        if target_concat1 is None:
                            target_concat1 = residual_total1.narrow(0,
                                                                    target_index[index] + residual_total1.size(0) / 2,
                                                                    1)
                        else:
                            target_concat1 = torch.cat((target_concat1,
                                                        residual_total1.narrow(0, target_index[
                                                            index] + residual_total1.size(0) / 2, 1)), 0)

                    conditional_loss1 = loss.CON(source_concat1, target_concat1)

                c1 = 1.0
                if (len(saveold) == lentarget):
                    c1=savecount[k]/float(max(savecount))
...
if __name__ == "__main__":
    ...
    transfer_classification(config, 'res50_amazon', sourcelist + ' to ' + targetlist, b,c)
